...
export default class ProductGridTable extends React.Component<IProductGridTable, any>{
    ...
    *,PlanOwner/Title,PlanOwner/Id,MarketPlanner/Title,MarketPlannerSupervisor/Title,RegionalSupplyLeader/Title,AboveMarketPlanner/Title,AboveMarketPlannerSupervisor/Title,
    ...
    public getGOLDStgListData = async () => {
        ...
        const items = await DataService.fetchAllItemsGenericFilter("Z_NPL_GOLD_Staging_List", '*', "IsActive eq 1 and IsMerged ne 1", 'Modified')
        ...
        await DataService.fetchAllDRListItemsWithFilters('DLPPList', `ID,DRID,Template,Country`, `DRID ne ${null}`, '', null).then(res => {
            ...
            filteredDlpp?.forEach(DlppItem => {
                filteredGolds?.forEach(goldItem => {
                    if (this.state.GOLDConfigData?.includes(DlppItem?.Template) && goldItem?.MappedDRID == DlppItem?.DRID?.toString() && goldItem.ProposedCountryCode == DlppItem.Country?.split("->")[0] && DlppItem.LaunchProgress !== "Cancelled") {
        ...
        return (
            <>
                <div style={{ marginTop: '-10px' }}>
                        ...
                        <Radio
                            name="DRData"
                            disabled={this.state.SelectedAIMode == 'View' || this.state.SelectedGOLDStgData.IntegrationStatus === 'Assigned' || this.state.SelectedGOLDStgData.IntegrationStatus === 'Published' || this.state.SelectedGOLDStgData.IntegrationStatus === 'Processed' || this.state.linkOrCreateDR === 'createDR' || rowData?.DRStatus === false}
                            value={rowData.DRID} onChange={(e) => this.onDRhandleChange(e, rowData)}
                            checked={(this.state.selectedDRID == rowData.DRID || this.state.SelectedGOLDStgData.IntegrationStatus === 'Assigned') || this.state.SelectedGOLDStgData.IntegrationStatus === 'Processed' || this.state.SelectedGOLDStgData.IntegrationStatus === 'Published'}
                            color='primary'
                        />
    ...
    public onConfirm = async () => {
        ...
        try {
            ...
            if(this.state.SelectedGOLDStgData.TradeName !== null) {
                ...
                filteredGoldItems = goldItems?.filter((item, i) => item.Molecule?.toLowerCase()?.includes(this.state.SelectedGOLDStgData.Molecule?.toLowerCase()) && item.TradeName === this.state.SelectedGOLDStgData.TradeName && item?.IntegrationStatus !== 'Assigned' && item?.IntegrationStatus !== 'Published' && item?.isDRPGSPlanExist === null);
            }
        ...
        this.checkAllIndicationExists(this.state.SelectedGOLDStgData.Indication?.split(';')).then(async res => {
            if (res) {
                ...
                if(this.state.SelectedGOLDStgData.TradeName !== null) {
                    ...
                    similarDRIDs = this.state.GOLDStgListData?.filter((item, i) => item.Molecule?.toLowerCase()?.includes(this.state.SelectedGOLDStgData.Molecule?.toLowerCase()) && item.TradeName === this.state.SelectedGOLDStgData.TradeName && item?.IntegrationStatus !== 'Assigned' && item?.IntegrationStatus !== 'Published' && item?.isDRPGSPlanExist === null);
                }
    ...
    protected onValueChangedVal = async (e: any) => {
        ...
        const goldItemsX = await DataService.fetchAllItemsGenericFilter("Z_NPL_GOLD_Staging_List", "*", "IsActive eq 1 and IsMerged ne 1", 'Modified');
        const goldItemsY = goldItemsX?.filter(item => item?.IsMerged != true);
        const order = { "New": 0, "Assigned": 1, "Published": 2, "Processed":3 };
    ...
    public confirmToCreateDR = async () => {
        if (this.state.SelectedMoleculeAPI != '' && this.state.SelectedGRP != '' && this.state.SelectedBU != '' && this.state.SelectedSubBU != '') {
            let projectDetailsListName = "";
            if (DataService.environment === "DEV") {
                projectDetailsListName = "ProjectDetailsList";
            }
            ...
            this.toast.show({ severity: 'warn', summary: 'Warning!', detail: "Please select the Mandatory fields", life: 4000 });
        }
    }
    ...
    public CheckTemplates = async (data) => {
        ...
        await DataService.fetchAllDRListItemsWithFilters('DLPPList', `ID,DRID,DLPPManaged,PlanStatus,Template,Country,LaunchProgress,ProjectName,PlanExistURL,PlanOwner/Title,PlanOwner/Id,PlanOwner/EMail`,
                        ...
                        <div className='row dx-Mockup-Background'>
                            <div className={this.state.opened ? 'col-12 rect demo-light' : 'col-12 rect1 demo-light'} style={!this.state.opened ? { marginTop: '-10px', marginLeft: '3%' } : {}}>
                                ...
                                >
                                    <div className="recordStatusOuterContainer">
                                        <div className='program-active-container'>
                                            {/* <DropDownButton
                                                ...
                                                text="Export"
                                            /> */}
                                            {!this.state.permissionError && this.state.SelectedTabName == "Launch List" && <Button className='p-button-raised p-button-rounded' style={{ width: "20rem", backgroundColor: "#001689", borderColor: "rgb(0 0 201)", float: 'left', display: 'flex', justifyContent: 'center' }}
                                            >
                                            ...
                                            {!this.state.permissionError && <Button title="Clear" className='p-button-raised p-button-rounded' style={{ backgroundColor: "#001689" }} onClick={(e) => this.globalFilterClear()} icon='pi pi-filter-slash'>
                                    ...
                                    {!this.state.permissionError ? <div style={this.state.ActiveIndex === 4 ? { "display": "none" } : { "height": "auto" }}>
                                    ...
                                    </div> : <div style={{ marginTop: '2rem' }}><h5>You don't have required permission, please contact Administrator !</h5></div>}
                                            ...
                                            {this.state.moleculeExisted ? <div className='CommercialDiv'>
                                                ...
                                                <div style={{ paddingTop: '20px', paddingBottom: '20px', paddingLeft: '3%' }}>
                                                    <DataGrid
                                                        ...
                                                        columnAutoWidth={true}
                                                        onCellPrepared={e => {
                                                            if (e?.rowType == 'data' && e?.data?.DRStatus == false) {
                                                                e.cellElement.style.backgroundColor = "#fee1e1";
                                                            }
                                                        }}
                                                    >
                                                        ...
                                                        <Column
                                                            cellRender={(celldata) => {
                                                                ...
                                                                return (
                                                                    ...
                                                                    <a onClick={() => window.open(href, target, 'noopener,noreferrer')} style={{ color: '#0d6efd' }}>{celldata.value}</a>
                                                                    {celldata?.data?.DRStatus === false && <span style={{ color: 'red' }}>Deleted!</span>}
                                                                    </div>
                                                                )
                                                            }}
                                            ...
                                            {this.state.moleculeExisted ? <div className='CommercialDiv'>
                                                ...
                                                <div style={{ paddingLeft: '3%' }}>
                                                    <Row style={{ paddingBottom: '2%' }}>
                                                        ...
                                                        <Col>
                                                            <Label>GRP[Global Reporting Product]<span style={{ color: 'red' }}>*</span></Label>
